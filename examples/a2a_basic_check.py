...
try:
    ...
    print(
        "   - EventQueue: Event handling for asynchronous agent communication"
    )
...
async def test_event_queue():
    ...
    try:
        ...
        print(
            f"   Queue closed: {queue.is_closed()} (作成直後 - まだ開いている)"
        )
        ...
        await queue.close()
        print(
            f"   Queue closed after close(): {queue.is_closed()} (正常にクローズされました)"
        )
...
async def main():
    ...
    print(
        "• 'Queue closed: False/True' - EventQueueのライフサイクルテスト（正常です）"
    )
    print("\n次のステップ:")
    print(
        "1. python -m app.a2a_prototype.test_simple_agent でエージェント機能をテスト"
    )
