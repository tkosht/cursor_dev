...
```'''
    ...
    def test_structure_compliance_violation(self, temp_project):
        ...
        with patch('pathlib.Path.cwd', return_value=temp_project):
            ...
            assert "未定義ディレクトリが存在: unauthorized_dir" in checker.violations[0]
...
多くの開発者がこの方法を採用しています。
'''
        (temp_project / "test_doc.md").write_text(doc_content, encoding='utf-8')
...
'''
    ...
    def test_git_unauthorized_changes_detection(self, temp_project):
        ...
        subprocess.run(['git', 'add', 'scripts/test.py'], 
        ...
        with patch('pathlib.Path.cwd', return_value=temp_project):
            ...
            assert result is False
            assert any("無許可新規ディレクトリ: scripts" in v for v in checker.violations)
    
    def test_exclude_directories_properly(self, temp_project):
        """除外ディレクトリの正常動作テスト"""
        # 除外対象ディレクトリを作成
        exclude_dirs = ['.git', '.venv', '__pycache__', 'node_modules', 'htmlcov']
...
標準的なREST API設計パターン。
'''
        (temp_project / "tech_spec.md").write_text(doc_content, encoding='utf-8')
        
        with patch('pathlib.Path.cwd', return_value=temp_project):
            checker = UserAuthorizationChecker()
            result = checker.check_evidence_based_claims()
            # 技術仕様の表現は許可されるべき
            technical_violations = [v for v in checker.violations 
...
@pytest.mark.parametrize("pattern,text,should_match", [
    (r'広く(?:使われて|受け入れられて|採用されて)', '広く使われている手法', True),
    (r'一般的(?:に|な)(?!.*(?:形式|フォーマット|仕様|API|プロトコル))', '一般的な方法', True),
    (r'一般的(?:に|な)(?!.*(?:形式|フォーマット|仕様|API|プロトコル))', '一般的なJSON形式', False),
    (r'標準的(?:に|な)(?!.*(?:JSON|XML|HTTP|REST|API|形式|仕様|プロトコル))', '標準的な手法', True),
    (r'標準的(?:に|な)(?!.*(?:JSON|XML|HTTP|REST|API|形式|仕様|プロトコル))', '標準的なHTTP仕様', False),
])
def test_subjective_pattern_matching(pattern, text, should_match):
    """主観的表現パターンのマッチングテスト"""
    import re
    match = re.search(pattern, text)
    assert bool(match) == should_match, f"Pattern '{pattern}' on text '{text}' should {'match' if should_match else 'not match'}"