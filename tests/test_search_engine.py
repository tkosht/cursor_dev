...
@pytest.mark.skipif(
    not torch.cuda.is_available(), reason="GPUが利用できない環境"
)
...
def test_search_error(engine, sample_bookmarks):
    ...
    engine.add_bookmarks(sample_bookmarks)
    with patch.object(
        engine.index, "search", side_effect=Exception("検索エラー")
    ):
        with pytest.raises(SearchEngineError) as exc_info:
...
def test_gpu_cpu_switching(index_dir, sample_bookmarks):
    ...
    for (gpu_bookmark, gpu_score), (cpu_bookmark, cpu_score) in zip(
...
def test_index_rebuild(engine, sample_bookmarks):
    ...
    for (initial_bookmark, initial_score), (
        rebuilt_bookmark,
        rebuilt_score,
    ) in zip(initial_results, rebuilt_results):
...
def test_index_file_size(engine, large_sample_bookmarks):
    ...
    bookmarks_size = os.path.getsize(
        os.path.join(engine.index_dir, "bookmarks.npy")
    )
