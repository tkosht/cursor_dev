...
class TestSecurityChecker:
    ...
    def test_api_key_detection(
        self, security_checker: SecurityChecker, temp_file: str
    ) -> None:
        """APIキーの検出テスト。
    ...
    def test_openai_key_detection_ideal(
        ...
        test_cases = [
            ...
            # 環境変数代入風
            (
                f'os.environ["OPENAI_API_KEY"] = "{valid_key}"',
                True,
                "Assignment Style",
            ),
            ...
            (
                f"long_key = '{valid_key}extra'",
                True,
                "Too Long Key (Known Limitation)",
            ),
            # プレフィックスが違う
            (
                f"wrong_prefix = 'pk-{valid_key_chars}'",
                False,
                "Wrong Prefix pk-",
            ),
            (
                f"wrong_prefix = 'sk_{valid_key_chars}'",
                False,
                "Wrong Prefix sk_",
            ),
            # 形式が不完全 (ハイフンなし)
            (
                f"incomplete = 'sk{valid_key_chars}'",
                False,
                "Incomplete Format (no hyphen)",
            ),
            # 一般的な変数名
            ("variable = 'sk_variable_name'", False, "Looks like variable"),
            # テスト用文字列
            (
                "test_string = 'this is not a key sk-abcdefg'",
                False,
                "Plain text containing short prefix",
            ),
            (
                f"test_string = 'this is not a real key {valid_key[:20]}...'",
                False,
                "Plain text containing partial key",
            ),
            # 空文字列
        ...
        for content, should_detect, description in test_cases:
            ...
            found, findings = security_checker.scan_file(
                Path(temp_file)
            )  # Pass Path object
            assert found == should_detect, (
    ...
    def test_github_token_detection_ideal(
        ...
        test_cases = [
            ...
            (
                "short_token = 'github_pat_short'",
                False,
                "Too Short Token (github_pat)",
            ),
            ...
            (
                f"invalid_chars = '{valid_ghp[:-1]}_'",
                False,
                "Invalid Characters (ghp)",
            ),
            # Invalid length
            (
                f"invalid_length = '{valid_ghp}extra'",
                False,
                "Invalid Length - Too Long (ghp)",
            ),
            (
                f"invalid_length = '{valid_ghp[:-1]}'",
                False,
                "Invalid Length - Too Short (ghp)",
            ),
            # Variable names
            (
                "variable = 'ghp_variable_name'",
                False,
                "Looks like variable (ghp)",
            ),
            (
                "variable = 'github_pat_variable'",
                False,
                "Looks like variable (github_pat)",
            ),
            # Plain text
            (
                "test_string = 'this is not a token ghp_abcdefg'",
                False,
                "Plain text containing short prefix",
            ),
            (
                f"test_string = 'look at {valid_ghp[:15]}...'",
                False,
                "Plain text containing partial token",
            ),
        ]
    ...
    def test_edge_cases(
        ...
        test_cases = [
            ("", False, "Empty File"),
            (
                f"# api_key = '{valid_openai_key}'",
                True,
                "Commented Out OpenAI Key",
            ),
            (
                f"# api_key = '{dummy_generic_key}'",
                True,
                "Commented Out Generic API Key",
            ),
            (
                "api_key = 'abcd' \\\n          '1234efgh5678'",
                False,
            ...
            (
                f"password = '{dummy_unicode_password}'",
                True,
                "Password Detection",
            ),
    ...
    def test_mask_file(
        ...
        assert (
            dummy_key not in masked_content
        ), "Original key still present after mask"
        assert (
            expected_mask in masked_content
...
class TestMainFunctionIntegration:
    ...
    def test_main_staged_clean_file(self, git_repo: Path):
        ...
        (git_repo / "clean.md").write_text(
            "This file is clean.", encoding="utf-8"
        )
    ...
    def test_main_unstaged_secret_file(self, git_repo: Path):
        ...
        (git_repo / "secret_unstaged.md").write_text(
        ...
        assert (
            Path("clean.md")
        ).name in result.stdout  # Only checks the staged file
    ...
    def test_main_staged_secret_non_target_extension(self, git_repo: Path):
        """Tests main exits cleanly with secrets in non-target file types."""
        run_git_command("reset", cwd=git_repo)  # Reset staging area
        secret_content = f"api_key = '{self.VALID_OPENAI_KEY}'"
        (git_repo / "secret.json").write_text(
            secret_content, encoding="utf-8"
        )  # .json is not target
