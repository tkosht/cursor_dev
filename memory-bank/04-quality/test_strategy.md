...
### **絶対禁止事項**
1. **見せかけテスト**: 実際にコードパスを実行しないテストケースの作成
2. **無効なside_effect**: 実際の例外パスを通らないモック設定
3. **カバレッジ除外の濫用**: `pragma: no cover`の不適切な使用
4. **テスト依存関係**: 他のテストに依存してカバレッジを稼ぐ手法
5. **意図的な未テスト**: テストが困難な部分の意図的回避

### **強制検証プロセス**
1. **個別テスト実行**: 各テストクラスを単独で実行し、カバレッジを確認
2. **エラーパス検証**: 例外処理が実際に実行されることを確認
3. **モック妥当性**: side_effectが実際のコードパスと一致することを確認
4. **カバレッジ差分分析**: 全体テストと個別テストのカバレッジ差を分析

### **品質ゲート追加要件**
- 個別テストクラスのカバレッジが全体カバレッジの80%以上
- エラーハンドリングテストは実際の例外パスを通ること
- side_effectは実際のコード実行パスと一致すること
