...
from app.a2a_prototype.utils.config import A2AConfig
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                if response.status_code == 200:
                    ...
                    print(f"❌ エージェントカード取得失敗: HTTP {response.status_code}")
                    return False
        ...
        while True:
            try:
                ...
                if cmd == 'test' and len(parts) >= 2:
                    ...
                    print("❌ 不明なコマンドです。'test <agent>', 'send <agent> <message>', 'quit'")
...
async def main():
    ...
    if len(sys.argv) > 1:
        ...
        if sys.argv[1] == "weather-test":
            ...
            await client.send_message_to_agent("calculator", "5 + 3 を計算して")
        else:
            print("使用方法: python test_client.py [weather-test|calculator-test]")
    else:
        # 対話式モード
        client = A2ATestClient()
