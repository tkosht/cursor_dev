...
class CalculatorAgent(BaseA2AAgent):
    ...
    def get_skills(self) -> List[AgentSkill]:
        ...
        return [
            ...
            AgentSkill(
                ...
                examples=["x + 5 = 12 を解いて", "2x = 10 の解は？"],
            ),
        ]
    ...
    def basic_calculate(self, expression: str) -> str:
        ...
        try:
            ...
            # 計算実行
            result = eval(
                safe_expression, {"__builtins__": {}}, self.safe_functions
            )
            ...
            return (
                f"計算エラー: {expression} を計算できませんでした。({str(e)})"
            )
    ...
    def solve_simple_equation(self, equation: str) -> str:
        ...
        try:
            ...
            if match3:
                ...
                # 除算結果を適切に処理
                solution: Union[int, float]
                if b % a == 0:
                    solution = b // a  # 整数除算
                else:
                    solution = b / a   # 浮動小数点除算
                return f"方程式 {equation} の解: x = {solution}"
            ...
            task.artifacts = [
                {
                    "parts": [
                        {
                            "type": "text",
                            "text": "計算中にエラーが発生しました。計算式を確認してください。",
                        }
                    ]
                }
            ]
