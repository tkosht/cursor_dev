...
class AgentHealthCheck:
    ...
    @staticmethod
    async def check_all_agents_health(agent_configs: List[AgentConfig]) -> Dict[str, bool]:
        """
        全エージェントのヘルス状態を確認
        ...
        for config in agent_configs:
            health_results[config.name] = await AgentHealthCheck.check_agent_health(config.url)
