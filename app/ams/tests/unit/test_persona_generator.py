...
class TestPersonaGenerator:
                ...
                "persona_slots": [
                    {
                        ...
                        "network_position": {"centrality": 0.7, "clustering": 0.5}
                    },
                    {
                        "id": "persona_1",
                        ...
                        "sub_segment": None,
                        "micro_cluster": None,
                        "network_position": {"centrality": 0.4, "clustering": 0.6}
                    }
                ]
            },
            "network_topology": {
    ...
    @pytest.mark.asyncio
    async def test_generate_personas_structure(self, generator, sample_article, sample_analysis_results, sample_population_structure):
        ...
        with patch.object(generator, '_analyze_context') as mock_context:
            ...
            with patch.object(generator, '_design_population') as mock_population:
                mock_population.return_value = sample_population_structure
                
                with patch.object(generator, '_generate_individual_personas') as mock_individual:
                    mock_personas = [
                        PersonaAttributes(
                            age=32,
                            occupation="Healthcare IT Director",
                            interests=["AI", "healthcare innovation"],
                            values=["innovation potential", "evidence quality"],
                            influence_score=0.8,
    ...
    @pytest.mark.asyncio
    async def test_analyze_context_integration(self, generator, sample_article):
        """Test context analysis integration."""
        mock_analyzer_result = {
            "core_context": {"domain_analysis": {"primary_domain": "healthcare"}},
            "hidden_dimensions": {"test": "dimension"},
            "complexity_score": 0.8,
            "reach_potential": 0.7
        }
        
        with patch.object(generator.context_analyzer, 'analyze_article_context') as mock_analyze:
    ...
    @pytest.mark.asyncio
    async def test_design_population_integration(self, generator, sample_analysis_results):
        ...
        with patch.object(generator.population_architect, 'design_population_hierarchy') as mock_design:
            mock_design.return_value = mock_population_result
            
            result = await generator._design_population(sample_analysis_results, target_size=10)
    ...
    @pytest.mark.asyncio
    async def test_generate_individual_personas(self, generator, sample_analysis_results, sample_population_structure):
        ...
        mock_persona_data = {
            ...
            "information_preferences": ["medical journals", "clinical trials", "peer networks"],
        ...
        with patch.object(generator, '_generate_single_persona') as mock_single:
    ...
    @pytest.mark.asyncio
    async def test_generate_single_persona(self, generator, sample_analysis_results):
        ...
        mock_response = {
            ...
            "information_preferences": ["tech publications", "medical journals", "industry reports"],
            "article_relationship": {
    ...
    def test_convert_to_persona_attributes(self, generator):
        ...
        persona_data = {
            ...
            "interests": ["medical imaging", "AI diagnostics", "patient safety"],
            "decision_factors": ["accuracy", "evidence", "patient impact"],
            "information_preferences": ["medical journals", "news websites", "professional networks"],
            "network_position": {
                "centrality": 0.7
            },
            "article_relationship": {
        ...
        assert abs(result.influence_score - 0.65) < 0.01  # Allow for floating point precision
        
        # Personality traits should be mapped to Big Five model
        assert len(result.personality_traits) > 0  # Should have mapped some traits
        
        # Should have at least one information channel
        assert len(result.preferred_channels) > 0

    @pytest.mark.asyncio
    async def test_error_handling(self, generator, sample_article, sample_analysis_results):
    ...
    @pytest.mark.asyncio
    async def test_persona_count_compliance(self, generator, sample_article, sample_analysis_results):
