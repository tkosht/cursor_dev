...
async def test_simulation_lifecycle():
    ...
    async with httpx.AsyncClient() as client:
        ...
        for i in range(max_polls):
            status_response = await client.get(f"{base_url}/api/simulations/{simulation_id}/status")
            ...
            print(
                f"[{i+1}] Status: {status_data['status']}, Progress: {status_data['progress']:.2%}"
            )
        ...
        final_response = await client.get(f"{base_url}/api/simulations/{simulation_id}")
        ...
        if final_data["status"] == "completed":
            ...
            if results_response.status_code == 200:
                ...
                if "result" in results and results["result"]:
                    ...
                    print(
                        f"- Processing time: {result_data.get('processing_time_seconds', 0):.2f}s"
                    )
            else:
