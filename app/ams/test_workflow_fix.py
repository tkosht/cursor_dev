...
async def test_workflow_transitions():
    ...
    try:
        ...
        async for chunk in graph.astream(initial_state, config=config):
            ...
            if previous_phase == "reporting" and not any(
                t[1] == "completed" for t in actual_transitions
            ):
                # The workflow might have ended without a final transition
                # Check the final state
                final_chunk = chunk
                for _node_name, node_state in final_chunk.items():
                    if (
                        isinstance(node_state, dict)
                        and node_state.get("current_phase") == "completed"
                    ):
