...
from ..core.types import SimulationResult, SimulationStatus
...
class SimulationService:
    ...
    async def run_simulation(self, simulation_id: str) -> None:
        ...
        try:
            ...
            async for chunk in self.graph.astream(
                initial_state, config={"configurable": {"thread_id": simulation_id}}
            ):
                chunk_count += 1
                logger.info(
                    "Processing chunk %s for simulation %s: %s",
                    chunk_count,
                    simulation_id,
                    list(chunk.keys()),
                )
            ...
            await self._update_status(
                simulation_id, SimulationStatus.FAILED, simulation["progress"], error=str(e)
            )
    ...
    async def _process_workflow_update(self, simulation_id: str, update: dict[str, Any]) -> None:
    ...
    def _extract_simulation_result(
        self, simulation_id: str, final_state: ArticleReviewState
    ) -> SimulationResult:
