...
class TargetAudienceAnalyzer:
    ...
    async def analyze_audience(
        self, article_content: str, article_metadata: dict[str, Any] | None = None
    ) -> AudienceAnalysis:
        """
    ...
    async def _identify_segments(
        self, article_content: str, topics: list[str]
    ...
    def _calculate_reach_potential(self, segments: list[AudienceSegment]) -> float:
        ...
        total_weight = sum(
            size_weights.get(s.size_estimate, 0.5) * s.relevance_score for s in segments
        )
    ...
    def _calculate_engagement_distribution(
        ...
        return {s.name: s.engagement_potential / total_engagement for s in segments}
