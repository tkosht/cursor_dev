...
class NetworkEffectSimulator:
    ...
    async def build_network(
        self, personas: list[Any], article_analysis: dict[str, Any]  # PersonaAttributes objects
    ) -> None:
        """
    ...
    async def simulate_propagation(
        ...
        influence_map = dict.fromkeys(self.network_graph, 0.0)
            ...
            for node_id in current_wave:
                ...
                for edge in self.edges:
                    ...
                    prob = self._calculate_propagation_probability(
                        node, self.network_graph.get(target_id), edge, content_score
                    )
    ...
    async def _create_network_node(
        self, persona: Any, index: int, article_analysis: dict[str, Any]
    ) -> NetworkNode:
    ...
    async def _generate_network_edges(self, article_analysis: dict[str, Any]) -> None:
        ...
        for _i, source_id in enumerate(node_ids):
            ...
            potential_targets = [nid for nid in node_ids if nid != source_id]
            ...
            for target_id, score in target_scores[:num_connections]:
                edge = NetworkEdge(
                    ...
                    relationship_type=self._determine_relationship_type(
                        source_node, self.network_graph[target_id]
                    ),
                )
    ...
    def _calculate_amplification_factor(self, persona: Any, influence_type: InfluenceType) -> float:
    ...
    def _calculate_network_position(
        self, index: int, influence_score: float
    ) -> tuple[float, float]:
    ...
    def _calculate_connection_score(self, source: NetworkNode, target: NetworkNode) -> float:
        ...
        if source.influence_type == target.influence_type:
            ...
            source.influence_type == InfluenceType.AUTHORITY
            and target.influence_type == InfluenceType.ASPIRATIONAL
        ):
            type_compat = 0.9
    ...
    def _determine_relationship_type(self, source: NetworkNode, target: NetworkNode) -> str:
    ...
    def _identify_key_influencers(self, influence_map: dict[str, float]) -> list[str]:
        ...
        sorted_nodes = sorted(influence_map.items(), key=lambda x: x[1], reverse=True)
