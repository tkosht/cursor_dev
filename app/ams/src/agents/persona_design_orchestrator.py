...
from src.core.types import PersonaAttributes, PersonalityType, InformationChannel
from src.utils.llm_factory import create_llm
from src.utils.json_parser import parse_llm_json_response
from src.agents.target_audience_analyzer import TargetAudienceAnalyzer, AudienceSegment
...
class PersonaDesign:
    ...
    network_position: Optional[NetworkNode]
...
class PersonaDesignOrchestrator:
    ...
    async def orchestrate_persona_generation(
        ...
        audience_analysis = await self.audience_analyzer.analyze_audience(
            article_content,
            article_analysis.get("metadata")
        )
        
        # Step 2: Design persona distribution across segments
        distribution = await self._design_persona_distribution(
            audience_analysis,
            target_count
        )
    ...
    async def _generate_unique_persona(
        ...
        behaviors = audience_analysis.behavioral_patterns
        ...
        - personality_traits: openness, conscientiousness, extraversion, agreeableness, neuroticism (0-1 scores)
    ...
    def _create_persona_attributes(
        ...
        return PersonaAttributes(
            ...
            information_seeking_behavior=persona_data.get("information_seeking_behavior", "moderate"),
    ...
    async def _enhance_with_network_positions(
        self,
        personas: List[PersonaAttributes]
    ) -> List[PersonaAttributes]:
        ...
        if max_count > len(personas) * 0.2:
            ...
            for i, persona in enumerate(personas):
                if occupation_counts.get(persona.occupation, 0) > 2:
                    persona.occupation = f"{persona.occupation} - {['Senior', 'Junior', 'Lead', 'Principal'][i % 4]}"
    ...
    def _create_fallback_persona_data(
        self,
        segment: AudienceSegment,
        index: int
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "values": segment.key_characteristics[:3] if segment.key_characteristics else ["growth", "learning"],
            "interests": segment.interests[:4] if segment.interests else ["technology", "innovation"],
            "personality_traits": {
                "openness": 0.6,
