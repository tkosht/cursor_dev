...
class PersonaGenerator:
    ...
    async def generate_personas(
        ...
        try:
            ...
            population_structure = await self._design_population(
                analysis_results, target_size=count
            )

            # Stage 3: Generate individual personas
            personas = await self._generate_individual_personas(
                analysis_results, population_structure
            )
        ...
        for slot in persona_slots:
            try:
                ...
                persona_data = await self._generate_single_persona(
                    slot, segment_info, analysis_results
                )
